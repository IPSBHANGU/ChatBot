...
- (void)setupImageView {
    ...
    CGFloat cornerRadius = CGRectGetWidth(_imageView.frame) / 2.0f - 2.0f;
...
- (CGPathRef)newBubbleWithRect:(CGRect)rect {
    ...
    CGPathAddLineToPoint(path, NULL, parentX - arrowWidth / 2.0f, rect.origin.y + rect.size.height);
    CGPathAddLineToPoint(path, NULL, parentX, rect.origin.y + rect.size.height + arrowHeight);
    CGPathAddLineToPoint(path, NULL, parentX + arrowWidth / 2.0f, rect.origin.y + rect.size.height);
